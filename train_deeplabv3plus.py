...
from model.deeplabv3plus import Deeplabv3plus   # <-- 按你的目录结构调整
...
def parse_arguments():
    ...
    parser.add_argument('--model', type=str, default='deeplabv3plus', help='Segmentation model')
...
def compute_class_weights(train_loader, num_classes, ignore_index=0):
    ...
    # Cap to avoid instability on ultra-rare classes
...
def train(args):
    ...
    # --- Model (DeepLabV3+ with ImageNet-pretrained backbone) -----------------
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if miou > best_miou:
            best_miou = miou
            ckpt_path = os.path.join(args.checkpoint, f'{args.model}_{args.backbone}_best.pth')
            torch.save({
                'epoch': epoch + 1,
